...
def get_auto_gen_disclaimer() -> str:
    return f"This file is auto-generated by 'repo_tools.py' in " \
           f"'[https://github.com/johannesgrothe/Smarthome_Bridge](Smarthome_Bridge)'"
            ...
            for category in api_spec:
                ...
                for title in api_spec[category]:
                    ...
                    if path_data["input_schema"]:
                        input_schema_str = json.dumps(shorten_json_schema(path_data["input_schema"]), indent=4,
                                                      sort_keys=True)
                        file.write(f"```json\n{input_schema_str}\n```\n")
                    else:
                        file.write("```json\n{}\n```\n")
                    ...
                    if path_data["output_schema"]:
                        output_schema_str = json.dumps(shorten_json_schema(path_data["output_schema"]), indent=4,
                                                       sort_keys=True)
                        file.write(f"```json\n{output_schema_str}\n```\n")
                    else:
                        file.write("```json\n{}\n```\n")
...
if __name__ == "__main__":
    if not os.path.isdir("../temp"):
        os.mkdir("../temp")
    generate_api_doc("../api.py", os.path.join("../temp", "api.md"))
